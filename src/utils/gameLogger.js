// Game logging utilities for export and database storage

/**
 * Format game log as human-readable text
 * @param {Array} gameLog - Array of game events
 * @param {Object} gameOptions - Game configuration
 * @returns {string} - Formatted text log
 */
export const formatGameLogAsText = (gameLog, gameOptions = {}) => {
    ...
    gameLog.forEach(event => {
        ...
        if (event.event === 'ROUND_START' && event.round !== currentRound) {
            ...
            // Show active players
            const activePlayers = event.players.filter(p => p.active);
        ...
        if (event.event === 'CHALLENGE') {
            ...
            event.allDice.forEach(p => {
                const diceStr = p.dice.map(d => `[${d}]`).join(' ');
 ...
 */
export const formatGameLogAsJSON = (gameLog, gameOptions = {}, roomId = null) => {
    const gameData = {
        gameId: roomId,
 ...
 */
export const downloadGameLog = (content, filename, mimeType = 'text/plain') => {
