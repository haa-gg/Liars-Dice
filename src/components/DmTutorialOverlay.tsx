...
const STEPS: StepDef[] = [
    // 0 – Welcome
    {
        title: '📜 DM Tutorial: Honor System Cheats',
        message: (
            <>
                ...
                <br /><br />
                {'Before starting any real game, open ⚙ Settings (top bar) and flip "Honor System Cheats" to On — the toggle locks once the round begins.'}
            </>
        ),
        showContinue: true,
    ...
    // 3 – Peek: target picker
    {
        ...
        message: (
            <>
                {"The player sees a list of active opponents. They tap one to spy on. In a TTRPG, tie this to a Deception or Perception check. Their potential victim would make a perception or insight check to detect the cheat."}
                <br /><br />
    ...
    // 4 – Peek: result (auto-advances)
    {
        ...
        message: (
            <>
                {"One die is shown from the target's cup — privately. No other player sees it. Pure intelligence gathering with minimal visible tell. Let's assume you succeeded on your check here."}
            </>
        ),
        showContinue: false,
    },
    // 5 – Slip
    {
        ...
        message: (
            <>
                {'"Slip" secretly adds an extra die to your hand without alerting the table assuming you succeeded on your check. This makes opponents guessing your hand much trickier.'}
            </>
        ),
        showContinue: true,
        ...
        message: (
            <>
                <ul style={{ textAlign: 'left', margin: '0 0 0.8rem 0', paddingLeft: '1.2rem' }}>
                    <li style={{ marginBottom: '0.4rem' }}><strong>Shield</strong> {"provides a crucial passive defense, automatically saving a die if the player loses a challenge."}</li>
    ...
    // 7 – Tabletop Tips
    {
        ...
        message: (
            <>
                {"Pro tip: When a player taps a cheat, have them roll the relevant skill check at the digital table. Success = use it; failure = getting busted-- does that mean a stern warning, a bottle broken over their heads, or something worse? Have fun with it!"}
            </>
        ),
        showContinue: true,
        continueLabel: 'Continue →',
    ...
    // 8 – Wrap-up
    {
        ...
        message: (
            <>
                {"If you're ever curious if a player used a cheat there's a public game log squirreled away in the hamburger menu during a game! This game handles the mechanics — you get to dole out the prizes/ consequences."}
            </>
        ),
        showContinue: true,
